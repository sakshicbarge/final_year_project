			...
			while (it2.hasNext()) {
				...
				if (key.contains(usName)) {
					...
					while (itFlMap.hasNext()) {
						...
						String tagwithPacket = mapping.split("_")[2].toString()
								.trim();
						int packNo = Integer
								.parseInt(tagwithPacket.split("@")[0]
										.toString().trim());
					...
					while (ii.hasNext()) {
						...
						blockNo = blockNo
								+ blckPacketMapIp.get(ip).keySet().toString()
										.replace("[", "").replace("]", "")
										.replace(",", "\n").replace(" ", "")
								+ "\n";
						packno = packno
								+ blckPacketMapIp.get(ip).values().toString()
										.replace("[", "").replace("]", "")
										.replace(",", "\n").replace(" ", "")
								+ "\n";
						Sign = Sign
								+ blckTagMapIp.get(ip).toString().replace("[",
										"").replace("]", "").replace(",", "\n")
										.replace(" ", "") + "\n";
						ipp = ipp + ip;
