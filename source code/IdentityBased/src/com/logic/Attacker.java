...
public class Attacker extends ActionSupport {
	...
	public String saveFile() {
		if (!serverUI.equalsIgnoreCase("----Select----")
				&& !blocks.equalsIgnoreCase("----Select----")
				&& !files.equalsIgnoreCase("----Select----")
				&& !editContent.equals("")) {
	...
	public String getFilesFromServer() {
		try {
			...
			} else if (!serverUI.equalsIgnoreCase("----Select----")
					&& !blocks.equalsIgnoreCase("----Select----")
					&& !files.equalsIgnoreCase("----Select----")) {
				String encContent = asc.AttackerFileFetch(serverUI, blocks,
						files);
				fileList.addAll(asc.AttackerFileListFetch(serverUI, blocks));
				// Base64 bs=new Base64();
				// content=new
				// String(bs.decode(encContent.split("\\@")[0].toString().trim().getBytes()));
